...
def test_fixed_status_options_ignore_custom_values():
    data = {
    ...
    assert clean["reagent_statuses"] == ["已订购", "可用", "停用", "已耗尽"]
    assert clean["validation_statuses"] == ["未验证", "通过", "不通过", "待复核"]
    assert clean["sample_statuses"] == ["可用", "停用", "已耗尽"]


def test_save_dropdown_options_persists_fixed_status_options(monkeypatch, tmp_path):
    ...
    assert saved["reagent_statuses"] == ["已订购", "可用", "停用", "已耗尽"]
    assert saved["validation_statuses"] == ["未验证", "通过", "不通过", "待复核"]
    assert saved["sample_statuses"] == ["可用", "停用", "已耗尽"]

    persisted = json.loads(options_path.read_text(encoding="utf-8-sig"))
    assert persisted["reagent_statuses"] == ["已订购", "可用", "停用", "已耗尽"]
