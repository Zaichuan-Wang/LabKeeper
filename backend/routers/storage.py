...
from core.security import require_admin, require_permission, require_user, visible_inventory_types
...
def storage_tree(user: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:
    return storage_api.storage_tree(visible_inventory_types(user))


@router.get("/storage/visual")
def storage_visual(request: Request, user: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:
    ...
    return storage_api.storage_visual(node_id, selected_well, item_type, item_id, visible_inventory_types(user))
